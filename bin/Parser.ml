type t =
  ...
  let skip n program_text =
    ...
    if new_position >= String.length line then
      { lines = program_text.lines; current_line = program_text.current_line + 1; current_position = 0 }
    else
      { lines = program_text.lines; current_line = program_text.current_line; current_position = new_position }

  let next program_text =
    skip 1 program_text
  ...
  let check_is_word word text =
    let rec loop text i =
      if i >= String.length word then
        ...
        | Some c when Char.lowercase_ascii c = Char.lowercase_ascii word.[i] -> loop (next text) (i + 1)
        | _ -> false
      in
    loop text 0
  ...
  let rec lex text res =
    ...
    | Some _ when check_is_word "endwhile" text -> lex (skip 8 text) (ENDWHILE :: res)
    | Some _ ->
  ...
  and consume_number text res = 
      ...
      | Some ' ' | Some ';' | None -> (res |> List.rev |> List.map (String.make 1) |> String.concat "" |> int_of_string, text)
      | Some _                     -> raise @@ LexerError ("Unexpected character while parsing number", text)

  and consume_string text res =
      match current text with
      | Some '"' -> 
        if peek 1 text = Some ' ' || peek 1 text = Some ';' || peek 1 text = None then              (* after string you can only have whitespace, ; or EOF *)
          (res |> List.rev |> List.map (String.make 1) |> String.concat "", next text)
        else
          raise @@ LexerError ("Unexpected character after string", text)
      | Some (_ as x) -> consume_string (next text) (x :: res)
      | None -> raise @@ LexerError ("Unexpected end of file while parsing string", text)

  and consume_word text res =
    match current text with
    | Some ' ' | Some ';' | None -> (res |> List.rev |> List.map (String.make 1) |> String.concat "", text)
